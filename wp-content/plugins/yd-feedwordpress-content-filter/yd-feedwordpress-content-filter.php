...
class yd_fwp_filter {
  ...
  function filter( $post ) {
    ...
    //Strip title and move link to syndicated_permalink
    if (preg_match('/<a\s[^>]*href\s*=\s*(\"??)([^\" >]*?)\\1[^>]*>(.*)<\/a>/siU', str_replace(', ”newwindow.html”','',htmlspecialchars_decode($title)).'</a>', $matches)):
      $title = $post['post_title'] = $matches[3];
      $post['meta']['syndication_permalink'] = $matches[2];
    endif;
    ...
    if( $options['local_images'] ) {
      ...
      if( preg_match( '/<img[^>]+src\s*=\s*["\']?([^"\' ]+)[^>]*>/', $stripped_content, $matches ) ) {
        $imgsrc = $matches[1];
        if( 1 == 1 ) {
          $imgext = 'jpg';  //strtolower( $matches[1] );
  ...
  function fixAttach( $postid ) {
    ...
    if( strlen( $mypost->post_name ) > 0 ) {
      ...
      foreach ( $attachments as $attachment ) {
        ...
        $saveId = update_post_meta($postid, '_thumbnail_id', $attachment->ID);
      }

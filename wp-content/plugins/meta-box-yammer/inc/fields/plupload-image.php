...
if ( ! class_exists( 'RWMBY_Plupload_Image_Field' ) )
{
	class RWMBY_Plupload_Image_Field extends RWMBY_Image_Field
	{
		/**
		 * Add field actions
		 *
		...
		static function handle_upload ()
		{
			...
			check_ajax_referer('rwmby-upload-images_' . $_REQUEST['field_id']);
		...
		static function admin_print_styles()
		{
			...
			wp_enqueue_style( 'rwmby-plupload-image', RWMBY_CSS_URL.'plupload-image.css', array(), RWMBY_VER );
			wp_enqueue_script( 'rwmby-plupload-image', RWMBY_JS_URL.'plupload-image.js', array( 'jquery-ui-sortable', 'wp-ajax-response', 'plupload-all' ), RWMBY_VER, true );
			...
			wp_localize_script( 'rwmby-plupload-image', 'rwmby_plupload_defaults', array(
				...
				'filters'				=> array( array( 'title' => _x( 'Allowed Image Files', 'image upload', 'rwmby' ), 'extensions' => 'jpg,gif,png,jpeg' ) ),
				'multipart'				=> true,
...
<li id='item_{$img_id}'>
	...
	<div class='rwmby-image-bar'>
		...
		<a title='{$i18n_del_file}' class='rwmby-delete-file' href='#' rel='{$img_id}'>{$i18n_delete}</a>
	</div>
</li>
HTML;
			return $html;
		...
		static function html( $html, $meta, $field )
		{
			...
			$i18n_drop		= apply_filters( 'rwmby_upload_drop_string', _x( 'Drop images here', 'image upload', 'rwmby' ) );
			$i18n_or        = _x( 'or', 'image upload', 'rwmby' );
			$i18n_select	= _x( 'Select Files', 'image upload', 'rwmby' );
			$img_prefix		= $field['id'];
			...
			$html .= wp_nonce_field( "rwmby-upload-images_{$field['id']}", "nonce-upload-images_{$field['id']}", false, false );
			$html .= "<input type='hidden' class='field-id rwmby-image-prefix' value='{$field['id']}' />";

			//Uploaded images
			$html .= "<div id='{$img_prefix}-container'>";
			$html .= "<h4 class='rwmby-uploaded-title'>{$i18n_msg}</h4>";
			...
			$html .= "
			<h4>{$i18n_title}</h4>
			<div id='{$img_prefix}-dragdrop' class='rwmby-drag-drop hide-if-no-js'>
				<div class = 'rwmby-drag-drop-inside'>
					<p>{$i18n_drop}</p>
					<p>{$i18n_or}</p>

jQuery( document ).ready( function ($)
{
	...
	{
		var	$input      = $( this ).parents( '.rwmby-input' );
			$clone_last = $input.find( '.rwmby-clone:last' ),
	...
	// Remove clones
	$( '.rwmby-input' ).delegate( '.remove-clone', 'click', function()
	{
		var $this  = $( this ),
			$input = $this.parents( '.rwmby-input' );

		// Remove clone only if there're 2 or more of them
		if ( $input.find( '.rwmby-clone' ).length > 1 )
		{
			$this.parent().remove();

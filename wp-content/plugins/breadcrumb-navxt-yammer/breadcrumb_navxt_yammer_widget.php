...
*/
class bcn_yammer_widget extends WP_Widget
{
	//Default constructor
	function __construct()
	{
		...
		if(!isset($l10n['breadcrumb_navxt_yammer']))
		{
			load_plugin_textdomain('breadcrumb_navxt_yammer', false, 'breadcrumb_navxt_yammer/languages');
		}
		$ops = array('classname' => 'widget_breadcrumb_navxt_yammer', 'description' => __('Adds a breadcrumb trail to your sidebar'));
		parent::__construct('bcn_yammer_widget', 'Breadcrumb NavXT', $ops);
	}
	function widget($args, $instance)
	{
		...
		if($instance['type'] == 'list')
		{
			...
			bcn_yammer_display_list(false, $instance['linked'], $instance['reverse']);
			echo '</ol>';
		}
		else if($instance['type'] == 'microdata')
		{
			...
			bcn_yammer_display(false, $instance['linked'], $instance['reverse']);
			echo '</div>';
		}
		else
		{
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"> <?php _e('Title:', 'breadcrumb_navxt_yammer'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('type'); ?>"> <?php _e('Output trail as:', 'breadcrumb_navxt_yammer'); ?></label>
			<select name="<?php echo $this->get_field_name('type'); ?>" id="<?php echo $this->get_field_id('type'); ?>">
				<option value="list" <?php selected('list', $instance['type']);?>><?php _e('List', 'breadcrumb_navxt_yammer'); ?></option>
				<option value="microdata" <?php selected('microdata', $instance['type']);?>><?php _e('Schema.org', 'breadcrumb_navxt_yammer'); ?></option>
				<option value="plain" <?php selected('plain', $instance['type']);?>><?php _e('Plain', 'breadcrumb_navxt_yammer'); ?></option>
			</select>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('linked'); ?>"> <?php _e('Link the breadcrumbs', 'breadcrumb_navxt_yammer'); ?></label><br />
			...
			<label for="<?php echo $this->get_field_id('reverse'); ?>"> <?php _e('Reverse the order of the trail', 'breadcrumb_navxt_yammer'); ?></label><br />
			...
			<label for="<?php echo $this->get_field_id('front'); ?>"> <?php _e('Hide the trail on the front page', 'breadcrumb_navxt_yammer'); ?></label><br />
		</p>
		<?php
	}
}
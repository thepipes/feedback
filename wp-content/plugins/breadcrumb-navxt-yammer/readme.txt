...
* Behavior change: `bcn_yammer_display_nested` function was removed from the main plugin. Will appear in a supplementary plugin.
...
= 3.5.0 =
* New feature: Added actions `bcn_yammer_before_fill` and `bcn_yammer_after_fill`, see documentation for more information.
...
= 3.4.1 =
* Bug fix: Fixed issue with PHP unexpected $end on line 1567 in breadcrumb_navxt_yammer_admin.php.
...
* Bug fix: The `bcn_yammer_display()` and `bcn_yammer_display_list()` wrapper functions obey the
`$return parameter`.
* Bug fix: Anchors now will be valid HTML even when a page/category/post title has HTML tags in it.
* Bug fix: Revised `bcn_yammer_breadcrumb_trail::category_parents` to work around a bug in `get_category` that causes a WP_Error to be thrown.
...
* Bug fix: Calling `bcn_yammer_display()` works with the same inputs as `bcn_yammer_breadcrumb_trail::display()`.
* Bug fix: Calling `bcn_yammer_display()` multiple times will not place duplicate breadcrumbs into the trail.
= 3.1.0 =
...
* Bug fix: Now `bcn_yammer_breadcrumb_trail::fill()` is safe to call within the loop.

...
class bcn_yammer_breadcrumb
{
	 ...
	 */
	public function bcn_yammer_breadcrumb($title = '', $template = '', $type = '', $url = NULL)
	{
		//Set the title
		...
		if($template == NULL)
		{
			$template = __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer');
		}
		if($url == NULL)
		{
	...
	{
		//Set the title
		$this->title = apply_filters('bcn_yammer_breadcrumb_title', __($title, 'breadcrumb_navxt_yammer'));
	}
	/**
	...
	{
		$this->url = apply_filters('bcn_yammer_breadcrumb_url', __($url, 'breadcrumb_navxt_yammer'));;
...
//The trail class
class bcn_yammer_breadcrumb_trail
{
	//Our member variables
	...
	public $opt;
	//Default constructor
	function bcn_yammer_breadcrumb_trail()
	{
		//Load the translation domain as the next part needs it		
		load_plugin_textdomain($domain = 'breadcrumb_navxt_yammer', false, 'breadcrumb-navxt-yammer/languages');
		...
		$this->opt = array(
			...
			'Smainsite_title' => __('Home', 'breadcrumb_navxt_yammer'),
			//The breadcrumb template for the main site, this is global, four keywords are available %link%, %title%, %htitle%, and %type%
			'Hmainsite_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Shome_title' => __('Home', 'breadcrumb_navxt_yammer'),
			//The breadcrumb template for the home page, this is global, four keywords are available %link%, %title%, %htitle%, and %type%
			'Hhome_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hblog_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			//'Hcurrent_item_template' => __('<a title="Reload the current page." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hpost_page_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hpaged_template' => __('Page %htitle%', 'breadcrumb_navxt_yammer'),
			...
			'Hpost_post_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hpost_attachment_template' => __('<a title="Go to %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'S404_title' => __('404', 'breadcrumb_navxt_yammer'),
			//Search page options
			//The breadcrumb template for search breadcrumbs, four keywords are available %link%, %title%, %htitle%, and %type%
			'Hsearch_template' => __('Search results for &#39;<a title="Go to the first page of search results for %title%." href="%link%">%htitle%</a>&#39;', 'breadcrumb_navxt_yammer'),
			...
			'Hsearch_template_no_anchor' => __('Search results for &#39;%htitle%&#39;', 'breadcrumb_navxt_yammer'),
			//Tag related stuff
			//The breadcrumb template for tag breadcrumbs, four keywords are available %link%, %title%, %htitle%, and %type%
			'Hpost_tag_template' => __('<a title="Go to the %title% tag archives." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hauthor_template' => __('Articles by: <a title="Go to the first page of posts by %title%." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hauthor_template_no_anchor' => __('Articles by: %htitle%', 'breadcrumb_navxt_yammer'),
			...
			'Hcategory_template' => __('<a title="Go to the %title% category archives." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
			...
			'Hdate_template' => __('<a title="Go to the %title% archives." href="%link%">%htitle%</a>', 'breadcrumb_navxt_yammer'),
	 ...
	 * @param bcn_yammer_breadcrumb $object Breadcrumb to add to the trail
	 */
	function &add(bcn_yammer_breadcrumb $object)
	{
		$this->trail[] = $object;
	...
	function do_search()
	{
		...
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_search_query(), $this->opt['Hsearch_template_no_anchor'], array('search', 'current-item')));
	...
	function do_author()
	{
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb(apply_filters('the_author', $curauth->$author_name), $this->opt['Hauthor_template_no_anchor'], array('author', 'current-item')));
			//If we're paged, or allowing the current item to be linked, let's link to the first page
	...
	function post_taxonomy($id, $type, $parent = null)
	{
		...
		if($this->opt['bpost_' . $type . '_taxonomy_display'])
		{
			...
			else if(is_taxonomy_hierarchical($this->opt['Spost_' . $type . '_taxonomy_type']))
			{
				...
				$bcn_yammer_object = get_the_terms($id, $this->opt['Spost_' . $type . '_taxonomy_type']);
				if(is_array($bcn_yammer_object))
				{
					//Now find which one has a parent, pick the first one that does
					$bcn_yammer_use_term = key($bcn_yammer_object);
					foreach($bcn_yammer_object as $key=>$object)
					{
						//We want the first term hiearchy
						if($object->parent > 0)
						{
							$bcn_yammer_use_term = $key;
							//We found our first term hiearchy, can exit loop now
							break;
						}
					}
					//Fill out the term hiearchy
					$this->term_parents($bcn_yammer_object[$bcn_yammer_use_term]->term_id, $this->opt['Spost_' . $type . '_taxonomy_type']);
				}
			}
			...
			else if(is_post_type_hierarchical($this->opt['Spost_' . $type . '_taxonomy_type']))
			{
				...
				$bcn_yammer_frontpage = get_option('page_on_front');
				//If there is a parent page let's find it
				if($parent && $id != $parent && $bcn_yammer_frontpage != $parent)
				{
					$this->post_parents($parent, $bcn_yammer_frontpage);
				}
			}
	...
	function post_terms($id, $taxonomy)
	{
		...
		if(is_array($bcn_yammer_object))
		{
			...
			foreach($bcn_yammer_object as $term)
			{
				...
				if($is_first == false)
				{
					$bcn_yammer_breadcrumb->set_title($bcn_yammer_breadcrumb->get_title() . ', ');
				}
				//This is a bit hackish, but it compiles the term anchor and appends it to the current breadcrumb title
				$bcn_yammer_breadcrumb->set_title($bcn_yammer_breadcrumb->get_title() . str_replace(
		...
		else
		{
			...
			$bcn_yammer_breadcrumb->set_title(__('Un' . $taxonomy, 'breadcrumb_navxt_yammer'));
		}
	}
	/**
	 * A Breadcrumb Trail Filling Function
	...
	function term_parents($id, $taxonomy)
	{
		...
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb($term->name, $this->opt['H' . $taxonomy . '_template'], array($taxonomy), get_term_link($term, $taxonomy)));
	...
	function post_parents($id, $frontpage)
	{
		...
		$parent = get_post($id);
		//Place the breadcrumb in the trail, uses the constructor to set the title, template, and type, get a pointer to it in return
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_title($id), $this->opt['Hpost_' . $parent->post_type . '_template'], array($parent->post_type), get_permalink($id)));
		//Make sure the id is valid, and that we won't end up spinning in a loop
	...
	{
		global $post, $page;
		//Place the breadcrumb in the trail, uses the bcn_yammer_breadcrumb constructor to set the title, template, and type
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_title(), $this->opt['Hpost_' . $post->post_type . '_template_no_anchor'], array('post-' . $post->post_type, 'current-item')));
		...
		if(is_post_type_hierarchical($post->post_type))
		{
			...
			$bcn_yammer_frontpage = get_option('page_on_front');
			//If there is a parent page let's find it
			if($post->post_parent && $post->ID != $post->post_parent && $bcn_yammer_frontpage != $post->post_parent)
			{
				$this->post_parents($post->post_parent, $bcn_yammer_frontpage);
			}
		}
	...
	function do_attachment()
	{
		global $post;
		//Place the breadcrumb in the trail, uses the constructor to set the title, template, and type, get a pointer to it in return
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_title(), $this->opt['Hpost_attachment_template_no_anchor'], array('post-attachment', 'current-item')));
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, template, and type, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_title($post->post_parent), $this->opt['Hpost_' . $post->post_type . '_template'], array($post->post_type), get_permalink($post->post_parent)));
			//Handle the post's taxonomy
	...
	function do_archive_by_term()
	{
		...
		//Place the breadcrumb in the trail, uses the constructor to set the title, template, and type, get a pointer to it in return
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb($term->name, $this->opt['H' . $term->taxonomy . '_template_no_anchor'], array($term->taxonomy, 'current-item')));
	...
	function do_archive_by_date()
	{
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_time('d'), $this->opt['Hdate_template_no_anchor'], array('date-day')));
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_time('F'), $this->opt['Hdate_template_no_anchor'], array('date-month')));
		...
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_time('Y'), $this->opt['Hdate_template_no_anchor'], array('date-year')));
	...
	{
		//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb(post_type_archive_title('', false), $this->opt['Hpost_' . get_post_type() . '_template_no_anchor'], array('post-' . get_post_type() . '-archive', 'current-item')));
		if($this->opt['bcurrent_item_linked'] || is_paged() && $this->opt['bpaged_display'])
		{
	...
	function do_front_page()
	{
		...
		//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
		$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->opt['Shome_title'], $this->opt['Hhome_template_no_anchor'], array('site-home', 'current-item')));
		//If we're paged, let's link to the first page
		if($this->opt['bcurrent_item_linked'] || (is_paged() && $this->opt['bpaged_display']))
		{
		...
		if($this->opt['bmainsite_display'] && !is_main_site())
		{
			...
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->opt['Smainsite_title'], $this->opt['Hmainsite_template'], array('mainsite-home'), get_home_url($current_site->blog_id)));
		}
	}
	/**
	 * A Breadcrumb Trail Filling Function
	...
	function do_home()
	{
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->opt['Shome_title'], $this->opt['Hhome_template'], array('site-home'), get_home_url()));
			//If we have a multi site and are not on the main site we need to add a breadcrumb for the main site
			if($this->opt['bmainsite_display'] && !is_main_site())
			{
				//Place the main site breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
				$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->opt['Smainsite_title'], $this->opt['Hmainsite_template_no_anchor'], array('mainsite-home'), get_home_url($current_site->blog_id)));
			}
		}
	}
	/**
	...
	function do_root()
	{
		...
		{
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->post_type_archive_title(get_post_type_object($type->post_type)), $this->opt['Hpost_' . $type->post_type . '_template'], array('post-' . $type->post_type . '-archive'), get_post_type_archive_link($type->post_type)));
		}
		else if(isset($type->taxonomy) && !$this->is_builtin($wp_taxonomies[$type->taxonomy]->object_type[0]) && $this->opt['bpost_' . $wp_taxonomies[$type->taxonomy]->object_type[0] . '_archive_display'] && $this->has_archive($wp_taxonomies[$type->taxonomy]->object_type[0]))
		{
			...
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_yammer_breadcrumb($this->post_type_archive_title(get_post_type_object($post_type)), $this->opt['Hpost_' . $post_type . '_template'], array('post-' . $post_type . '-archive'), get_post_type_archive_link($post_type)));
		}
		...
		if(isset($posts_id) || $this->opt['bblog_display'] && get_option('show_on_front') == 'page' && (is_home() || is_single() || is_tax() || is_category() || is_tag()))
		{
			...
			{
				//Get the blog page
				$bcn_yammer_post = get_post($posts_id);
				...
				$breadcrumb = $this->add(new bcn_yammer_breadcrumb(get_the_title($posts_id), $this->opt['Hpost_' . $post->post_type . '_template_no_anchor'], array($type_str . '-root', 'post-' . $post->post_type)));
				...
				if($bcn_yammer_post->post_parent && $bcn_yammer_post->ID != $bcn_yammer_post->post_parent && $frontpage_id != $bcn_yammer_post->post_parent)
				{
					$this->post_parents($bcn_yammer_post->post_parent, $frontpage_id);
				}
			}
		}
	}
	...
	function do_404()
	{
		//Place the breadcrumb in the trail, uses the bcn_yammer_breadcrumb constructor to set the title, prefix, and suffix
		$this->trail[] = new bcn_yammer_breadcrumb($this->opt['S404_title'], $this->opt['H404_template'], array('404', 'current-item'));
	}
	/**
	 * A Breadcrumb Trail Filling Function
	 * 
	...
	function do_paged()
	{
		...
		//Place the breadcrumb in the trail, uses the bcn_yammer_breadcrumb constructor to set the title, prefix, and suffix
		$this->trail[] = new bcn_yammer_breadcrumb($page_number, $this->opt['Hpaged_template'], array('paged'));
	}
	/**
	...
	function fill()
	{
		...
		do_action('bcn_yammer_before_fill', $this);
		...
		//Do any actions if necessary, we past through the current object instance to keep life simple
		do_action('bcn_yammer_after_fill', $this);
	}
	/**

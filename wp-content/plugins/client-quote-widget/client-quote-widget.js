(function($){
	...
	$(document).ready(function() {
		...
		if ( typeof $.fn.on !== 'undefined' ) {
			$("#wpbody").on("click", ".thickbox-quote-widget", imgHandler);
		}
		else {
			$("a.thickbox-quote-widget").live('click', imgHandler);
		}
		
		// Modify thickbox link to fit window. Adapted from wp-admin\js\media-upload.dev.js.
		$('a.thickbox-quote-widget').each( function() {
...
jQuery(document).ready(function() {
  ...
  jQuery('form#image-form').submit(function(){
    ...
    if( wp_ref.indexOf('widget_sp_quote') != -1 ) {
      ...
      if( nw_action_url.indexOf('widget_sp_quote') == -1 ) {

...
function roots_get_default_theme_options($default_framework = '') {
  global $roots_css_frameworks;
  if ($default_framework == 'yamkit') { $default_framework = apply_filters('roots_default_css_framework', 'yamkit'); }
  ...
  $default_theme_options = array(
    ...
    'production_version'        => '',
    'contact_sales_form_id'     => 1,
    'contact_affiliates_form_id' => 2,
    'premium_groups_apr24'      => false,
    'quantcast_id'              => '',
  ...
  <div class="wrap">
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table">
        ...
        <?php if($roots_options['css_framework'] == 'yamkit') { ?>
        <tr valign="top"><th scope="row"><?php _e('Development', 'roots'); ?></th>
          <td>
            <fieldset class="roots_development"><legend class="screen-reader-text"><span><?php _e('Development Environment?', 'roots'); ?></span></legend>
              <select name="roots_theme_options[development]" id="roots_theme_options[development]">
                <option value="yes" <?php selected($roots_options['development'], true); ?>><?php echo _e('Yes', 'roots'); ?></option>
                <option value="no" <?php selected($roots_options['development'], false); ?>><?php echo _e('No', 'roots'); ?></option>
              </select>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Contact Sales Form ID', 'roots'); ?></th>
          <td>
            <fieldset class="roots_development"><legend class="screen-reader-text"><span><?php _e('Contact Sales Form ID', 'roots'); ?></span></legend>
              <input type="text" name="roots_theme_options[contact_sales_form_id]" id="contact_sales_form_id" value="<?php echo esc_attr($roots_options['contact_sales_form_id']); ?>" class="small-text" />
              <br />
              <small class="description"><?php _e('Default:', '1'); ?> <span><?php echo $roots_default_options['contact_sales_form_id']; ?></span></small>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Contact Affiliates Form ID', 'roots'); ?></th>
          <td>
            <fieldset class="roots_development"><legend class="screen-reader-text"><span><?php _e('Contact Affiliates Form ID', 'roots'); ?></span></legend>
              <input type="text" name="roots_theme_options[contact_affiliates_form_id]" id="contact_affiliates_form_id" value="<?php echo esc_attr($roots_options['contact_affiliates_form_id']); ?>" class="small-text" />
              <br />
              <small class="description"><?php _e('Default:', '2'); ?> <span><?php echo $roots_default_options['contact_affiliates_form_id']; ?></span></small>
            </fieldset>
          </td>
        </tr>
        <?php } ?>
        ...
        <tr valign="top"><th scope="row"><?php _e('Production Version', 'roots'); ?></th>
            <td>
                <fieldset><legend class="screen-reader-text"><span><?php _e('Production Version', 'roots'); ?></span></legend>
                    <input type="text" name="roots_theme_options[production_version]" id="production_version" value="<?php echo esc_attr($roots_options['production_version']); ?>" />
                    <br />
                    <small class="description"><?php printf(__('Enter the production release version. E.g. 1.0', 'roots')); ?></small>
                </fieldset>
            </td>
        </tr>
        
        <tr valign="top"><th scope="row"><?php _e('Premium Groups Launched?', 'roots'); ?></th>
          <td>
            <fieldset class="roots_yamkit"><legend class="screen-reader-text"><span><?php _e('Premium Groups Launched?', 'roots'); ?></span></legend>
              <select name="roots_theme_options[premium_groups_apr24]" id="roots_theme_options[premium_groups_apr24]">
                <option value="yes" <?php selected($roots_options['premium_groups_apr24'], true); ?>><?php echo _e('Yes', 'roots'); ?></option>
                <option value="no" <?php selected($roots_options['premium_groups_apr24'], false); ?>><?php echo _e('No', 'roots'); ?></option>
              </select>
            </fieldset>
          </td>
        </tr>

        <tr valign="top"><th scope="row"><?php _e('Quantcast ID', 'roots'); ?></th>
            <td>
                <fieldset><legend class="screen-reader-text"><span><?php _e('Quantcast ID', 'roots'); ?></span></legend>
                    <input type="text" name="roots_theme_options[quantcast_id]" id="quantcast_id" value="<?php echo esc_attr($roots_options['quantcast_id']); ?>" />
                    <br />
                    <small class="description"><?php printf(__('Enter the Quantcast ID for tracking here, ex: p-ccam0jTDUaPC2', 'roots')); ?></small>
                </fieldset>
            </td>
        </tr>

        <tr valign="top"><th scope="row"><?php _e('Display Annoucement?', 'roots'); ?></th>
          <td>
            <fieldset class="roots_yamkit"><legend class="screen-reader-text"><span><?php _e('Display Annoucement?', 'roots'); ?></span></legend>
              <select name="roots_theme_options[announcement_on]" id="roots_theme_options[announcement_on]">
                <option value="no" <?php selected($roots_options['announcement_on'], false); ?>><?php echo _e('No', 'roots'); ?></option>
                <option value="yes" <?php selected($roots_options['announcement_on'], true); ?>><?php echo _e('Yes', 'roots'); ?></option>                
              </select>
            </fieldset>
          </td>
        </tr>

        <tr valign="top"><th scope="row"><?php _e('Announcement Description', 'roots'); ?></th>
            <td>
                <fieldset><legend class="screen-reader-text"><span><?php _e('Announcement Description', 'roots'); ?></span></legend>
                    <textarea name="roots_theme_options[announcement_desc]" id="announcement_desc"><?php echo esc_attr($roots_options['announcement_desc']); ?></textarea> 
                    <br />
                    <small class="description"><?php printf(__('Enter the announcement description.', 'roots')); ?></small>
                </fieldset>
            </td>
        </tr>
      </table>
...
function roots_theme_options_validate($input) {
  ...
  if (isset($input['development'])) {
    if ($input['development'] === 'yes') {
      $input['development'] = true;
    }
    if ($input['development'] === 'no') {
      $input['development'] = false;
    }
    $output['development'] = $input['development'];
  }

  if (isset($input['premium_groups_apr24'])) {
    if ($input['premium_groups_apr24'] === 'yes') {
      $input['premium_groups_apr24'] = true;
    }
    if ($input['premium_groups_apr24'] === 'no') {
      $input['premium_groups_apr24'] = false;
    }
    $output['premium_groups_apr24'] = $input['premium_groups_apr24'];
  }
  
  if (isset($input['contact_sales_form_id'])) {
    $output['contact_sales_form_id'] = intval($input['contact_sales_form_id']);
  }
  
  if (isset($input['contact_affiliates_form_id'])) {
    $output['contact_affiliates_form_id'] = intval($input['contact_affiliates_form_id']);
  }
  ...
  if (isset($input['production_version'])) {
    $output['production_version'] = $input['production_version'];
  }

  if (isset($input['quantcast_id'])) {
    $output['quantcast_id'] = $input['quantcast_id'];
  }

  if (isset($input['announcement_on'])) {
    if ($input['announcement_on'] === 'yes') {
      $input['announcement_on'] = true;
    }
    if ($input['announcement_on'] === 'no') {
      $input['announcement_on'] = false;
    }
    $output['announcement_on'] = $input['announcement_on'];
  }

  if (isset($input['announcement_desc'])) {
    $output['announcement_desc'] = $input['announcement_desc'];
  }

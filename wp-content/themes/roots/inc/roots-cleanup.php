...
function roots_tabmenu_classes_filter(&$val, $key, $selected_class){
	if (preg_match('/current-/', $val)){
		$val=$selected_class;
	} else if (!preg_match('/yj-/', $val)){
		$val = '';
	} 
}
...
class Tab_Menu_Walker extends Walker_Nav_Menu {
    ...
    $class_names = $value = '';
		$tab_li = 'yj-tab';
		$tab_li_selected= 'yj-selected';
		$tab_li_link = 'yj-tab-link';
		
    $classes = array_push($item->classes, $tab_li);
    $classes = empty($item->classes) ? array() : (array) $item->classes;

    array_walk($classes, 'roots_tabmenu_classes_filter', $tab_li_selected);
    $class_names = join(' ', apply_filters('nav_menu_css_class', array_filter($classes), $item));
    $class_names = $class_names ? ' class="' . esc_attr($class_names). '"' : '';
...
function roots_default_wp_nav_menu_walker($args = '') {
  global $roots_options;
  if ($args['tab_menu']) {
    $args['walker'] = new Tab_Menu_Walker();
  } else if ($roots_options['clean_menu']) {
